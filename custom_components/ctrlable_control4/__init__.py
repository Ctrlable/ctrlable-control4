...
async def get_items_of_category(hass,entry,category):
	...
	except InvalidCategory as D:_LOGGER.warning('Category %s does not exist on this Control4 system,                         entities from this domain will not be setup.',A,exc_info=_B);return[]
async def refresh_tokens(hass,entry):
	...
	H=aiohttp_client.async_get_clientsession(C,verify_ssl=_A);I=C4Director(D[CONF_HOST],G[CONF_TOKEN],H);_LOGGER.debug('Saving new account and director tokens in hass data');B=C.data[DOMAIN][E.entry_id];B[CONF_ACCOUNT]=F;B[CONF_DIRECTOR]=I
...
class C4WebsocketConnectionTracker:
	def __init__(A,hass,entry):A.hass=hass;A.entry=entry;A._was_disconnected=_A
	async def connect_callback(A):
		...
		A._was_disconnected=_A
	async def disconnect_callback(A):
		_LOGGER.warning('Websocket connection to Control4 lost, attempting reconnection');A._was_disconnected=_B;B=A.hass.data[DOMAIN][A.entry.entry_id][CONF_WEBSOCKET].item_callbacks
		for(C,D)in B.items():await D(C,_A)
...
class Control4Entity(Entity):
	...
	async def _update_callback(A,device,message):
		...
		if B is _A:A._attr_available=_A
		elif B[_C]==_D:A._attr_available=_B;C=B['data'];await A._data_to_extra_state_attributes(C)
